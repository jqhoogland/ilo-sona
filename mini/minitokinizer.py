...
class Tokinizer:
<<<<<<< HEAD
    def __init__(self, vocabulary=VOCABULARY, pn_policy="characterize"):
        self.vocabulary = vocabulary
        self.tokens = ["<undefined>"] + vocabulary + LETTERS + PUNCTUATION + DIGITS + [" "]
        self.token_to_id = {token: i for i, token in enumerate(self.tokens)}
        self.id_to_token = {v: k for k, v in self.token_to_id.items()}
        self.vocab_size = len(self.tokens)
        self.pn_policy = pn_policy
...
<<<<<<< HEAD
            # We have the option of characterizing proper nouns or ignoring them.
            # Ignoring them amounts to replacing them with an <undefined> token.
            if (proper_noun & self.pn_policy != "ignore") or token.isnumeric():
=======
...
<<<<<<< HEAD
        # Collapse blocks of undefined tokens into one single token.
=======
...
if __name__ == "__main__":
    ...
    print("-"*30)
<<<<<<< HEAD
    tokitext = tokinizer.decode(tokinizer.encode(text)) # (reconstructed text)
    # print(tokitext)
    # print("-"*30)
    print(f"Test text reconstructed from tokinized text: {text == tokitext}")
=======
